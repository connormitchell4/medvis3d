...
export default function Volume3DView({
  ...
  // Heatmap helpers removed (unused)
  ...
  function applyDefaultCameraFacingAxial() {
    ...
    cam.setDirectionOfProjection(0, 0, 1);
    // Look from +Z towards the axial slice
    cam.setPosition(-cx , 90, cz *2);
    // 180° roll about Z: flip the view-up vector
    cam.setViewUp(0, 1, 0);
  ...
  useEffect(() => {
    ...
    grw.getRenderWindow().render();
    // Re-apply default camera when segmentation is first added
    applyDefaultCameraFacingAxial();
    applyDefaultCameraFacingAxial();
